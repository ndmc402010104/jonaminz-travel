...
(function () {
  ...
  function emptyState() {
    return { version: 1, trips: [], places: [], days: [], stops: [], activeTripId: null, dismissedTemplateIds: [] };
  }
  ...
  function importTemplate(templateId) {
    ...
    if (!template || template.schemaVersion !== 1) return null;
    ...
    template.days.forEach(function (source) {
      ...
      state.days.push({
        ...
        index: source.index,
        date: source.date || "",
        title: source.title || ("Day " + source.index),
        templateKey: source.key
      });
    });
    ...
    template.days.forEach(function (source) {
      source.stops.forEach(function (item, index) {
        var place = template.places.filter(function (candidate) { return candidate.key === item.placeKey; })[0];
        if (!place || !placeIds[item.placeKey] || !dayIds[source.key]) return;
        state.stops.push({
          id: uid(),
  ...
  function deleteTrip(tripId) {
    ...
    if (existing.templateId && state.dismissedTemplateIds.indexOf(existing.templateId) === -1) {
      state.dismissedTemplateIds.push(existing.templateId);
    }
    ...
    return (
      '<div class="builder-hero">' +
        '<div>' +
          ...
          '<span class="phase-chip">' + (trip.templateId ? "PAST TRIP · 2025.10.24–10.26" : "PHASE 1 · 行程編排可用") + '</span>' +
        ...
        '<div class="builder-summary">' +
          ...
          '<button type="button" class="trip-delete" data-delete-trip="' + trip.id + '">刪除這趟旅行</button>' +
        "</div>" +
      "</div>"
    );
  }
  ...
  function bindEvents(root) {
    root.addEventListener("click", function (event) {
      ...
      var deleteTripBtn = event.target.closest("[data-delete-trip]");
      if (deleteTripBtn) {
        var approved = window.confirm("確定要刪除這趟旅行？其中的景點、日期與行程安排會一起刪除。");
        if (approved) { deleteTrip(deleteTripBtn.getAttribute("data-delete-trip")); render(root); }
        return;
      }
  ...
  function init() {
    state = loadState();
    if (!state.trips.length && TEMPLATES.length && state.dismissedTemplateIds.indexOf(TEMPLATES[0].templateId) === -1) {
      importTemplate(TEMPLATES[0].templateId);
    }
