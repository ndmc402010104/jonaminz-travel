...
(function () {
    ...
    return (
      '<div class="builder-hero">' +
        '<div>' +
          ...
          '<p>景點素材只保存一次；把它安排到某一天，之後就能建立行程、旅行本頁面與回憶位置（旅行書生成還沒做，先把安排行程這一步做穩）。</p>' +
  ...
  function renderAddPlaceForm() {
    ...
    return (
      '<form class="new-trip-form" data-new-place-form style="margin-bottom:13px">' +
  ...
  function renderAddDayForm() {
    ...
    return (
      '<form class="new-trip-form" data-new-day-form style="margin-bottom:13px">' +
      ...
      ? days.map(function (d) {
          ...
          var stopsHtml = stops.length
            ? stops.map(function (s, i) {
                return (
                  ...
                  '<button type="button" class="icon-btn" data-move="up" data-stop-id="' + s.id + '"' + (i === 0 ? " disabled" : "") + " title=\"往上移\">↑</button>" +
                  '<button type="button" class="icon-btn" data-move="down" data-stop-id="' + s.id + '"' + (i === stops.length - 1 ? " disabled" : "") + " title=\"往下移\">↓</button>" +
                  '<button type="button" class="icon-btn" data-unassign="' + s.id + '" title="移除">✕</button>' +
                  "</div></div>"
                );
              }).join("")
          ...
          return (
            '<div class="builder-day-column">' +
              '<header><div><small>DAY ' + d.index + '</small><h4>' + escapeHtml(d.title) + "</h4></div></header>" +
  ...
  function renderGenerationPanel() {
    return (
      '<aside class="generation-panel">' +
        '<header><div><small>BOOK GENERATION</small><h3>旅行書頁面同步狀態</h3></div></header>' +
        ...
        '<p class="generation-note">旅行書生成還沒做（見交接包 REBUILD_PLAN.md Phase 3）。這裡先把版面畫出來，排行程排完之後才輪到這一步。</p>' +
      "</aside>"
    );
  }
  ...
  function renderBoard(root) {
    ...
    if (!state.activeTripId) {
      boardEl.innerHTML = '<p class="empty-panel">先在上面建立一趟旅行，或選一個既有的，才能開始排行程。</p>';
      return;
    }
