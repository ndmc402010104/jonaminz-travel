...
## C8 — 2026-07-23 Responsive map interaction

- 驗收目標：
  - 手機「操作地圖」與「完成操作」的右上角定位不跳動。
  - 手機仍預設鎖定，避免地圖攔截垂直滑頁。
  - 桌機需同時符合寬度至少 981 px、可 hover、fine pointer，直接啟用地圖且不顯示操作按鈕。
  - 視窗或輸入裝置能力改變時，同步更新兩張地圖的互動狀態。
  - Google Places 評分資料只完成架構評估，不把金鑰加入靜態站。
- 驗證：JavaScript syntax；既有四組 DOM flow；mobile lock/unlock；desktop fine-pointer 自動啟用。
