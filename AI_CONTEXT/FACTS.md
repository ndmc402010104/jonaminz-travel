...
- localStorage key 為相容性保留；內部 state schema 已升級為 version 4。
- 已實作資料關係：Trip、Place、Day、Stop、Booking、Checklist、ShoppingItem；Memory 容器已保留。
...
- `templates.js` 目前提供 schemaVersion 3 的 `sapporo-2025-10` declarative historical trip pack，來源為使用者的 2025/10/24–10/26《北海道_札幌.pdf》；全新瀏覽器會將它建立為第一筆正式旅行。
- 產品有五個正式模式：旅行首頁、Journey Builder、Live Trip、Book Studio、探索工具。
- Stop 的 completed 用於 Live Trip 手動完成進度；Trip 的 bookStyle 用於旅行書風格。
