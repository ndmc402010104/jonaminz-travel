...
Jonathan 與 Minz 的旅行規劃：建立旅行、收集景點、排進每天、調整順序（MVP demo）。
...
目前只做「第一條垂直流程」（見 `jonaminz-travel-handoff-v1.0/AI_CONTEXT/
REBUILD_PLAN.md` Phase 1+2）。旅行書生成、Book Style、Book Studio、
...
demo 資料之後要漸進接上真實身分／後端，並往交接包 REBUILD_PLAN.md 的
Phase 3 以後（旅行書生成、Book Style、Book Studio、Live Trip）推進，
保留現有的視覺語言與互動方式。
